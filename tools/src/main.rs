...
fn execute_kernel_space_build_phase() {
    let workspace_root = PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        .parent()
        .expect("tools must be located directly under the workspace root")
        .to_path_buf();
    ...
    kernel_space_build_command.arg(workspace_root.join("bpf/Cargo.toml"));
    ...
    kernel_space_build_command.arg(workspace_root.join("target"));
